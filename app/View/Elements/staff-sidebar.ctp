...
<ul class="sidebar-menu folders">
	...
	if(isset($_folders) && is_array($_folders) && count($_folders)) {
		...
		?>
		<li><?php echo $this->Html->link(ucwords(strtolower($_folder['ManageFolder']['Name'].' ('.$_folder['ManageFolder']['count'].')')), array('controller' => 'uploads', 'action' => 'folder/'.$_folder['ManageFolder']['id'], 'admin' => false)); ?> </li>
		<?php
		}
	}
	?>
