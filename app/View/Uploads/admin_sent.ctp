					...
					foreach ($messages as $message):
						...
						<!--	<td><?php echo $this->Html->link($message['Document']['name'], '/app/webroot/files/uploads/' . $message['Document']['filename']); ?>&nbsp;</td>-->
							...
							<td class="actions">
								...
								<?php echo $this->Form->postLink('<button class="btn btn-danger btn-xs"><i class="fa fa-trash-o "></i></button>', array('action' => 'delete_sent', $message['Message']['id'], 'admin'=>false), array('escape' => false, 'title' => "Delete"), __('Are you sure you want to delete # %s?', $message['Message']['id'])); ?>
								<?php //} ?>
							</td>
						</tr>
					<?php
